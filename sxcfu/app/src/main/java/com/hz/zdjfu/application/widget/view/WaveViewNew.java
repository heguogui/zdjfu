...
public class WaveViewNew extends View {
    ...
    public static final int DEFAULT_BEHIND_WAVE_COLOR = Color.parseColor("#64b9fc");
    public static final int DEFAULT_FRONT_WAVE_COLOR = Color.parseColor("#64b9fc");
    ...
    public WaveViewNew(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public WaveViewNew(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void createShader() {
        ...
        Log.i("VaveViewNew","getWidth()="+getWidth()+"getHeight()="+getHeight());
        ...
        final int wave2Shift = (int) (mDefaultWaveLength/5); // 水波的高度
        for (int beginX = 0; beginX < endX; beginX=beginX+1) {
        ...
        if (mShowWave && mWaveShader != null) {
            ...
            switch (mShapeType) {
                case CIRCLE:
                    ...
                    float radius = getWidth() / 2f - (borderWidth+10);

                    if (borderWidth > 0) {
                        // 内部圆框
                        canvas.drawCircle(getWidth() / 2f, getHeight() / 2f,
                                radius+2, mBorderPaint);

                        // 外部包围的圆框
                        Paint  mCirclePaint = new Paint();
                        mCirclePaint.setAntiAlias(true);
                        mCirclePaint.setColor(Color.parseColor("#ff82c0ff"));
                        mCirclePaint.setStyle(Paint.Style.STROKE);
                        mCirclePaint.setStrokeWidth(15F);
                        canvas.drawCircle(getWidth() / 2f, getHeight() / 2f,
                                (getWidth()/ 2f - (borderWidth+5)), mCirclePaint);

                    }

                    Paint mCirclePaint = new Paint();
                    mCirclePaint.setAntiAlias(true);
                    mCirclePaint.setColor(Color.parseColor("#ff0066ff"));
                    mCirclePaint.setStyle(Paint.Style.FILL);
                    canvas.drawCircle(getWidth() / 2f, getHeight() / 2f, radius+2, mCirclePaint);
                    // 内部实心圆
                    canvas.drawCircle(getWidth() / 2f, getHeight() / 2f, radius+2, mViewPaint);
                    break;
                case SQUARE:
                    if (borderWidth > 0) {
                        canvas.drawRect(
